...
AS
	...
	SET @LOGSOURCE = N'STAGE.[gs].[CUSTOMER_CDC]';
	
	EXEC META.dbo.USP_WRITELOG N'Start to load [gs].[CUSTOMER_CDC]', @LOGSOURCE, N'N';
			...
			WHERE 1 = 1
				AND CASE
					...
					ELSE
						''
				END <> '';
			EXEC META.dbo.USP_WRITELOG N'Finish to load gs].[CUSTOMER_LOG', @LOGSOURCE, N'N';
		COMMIT TRAN;
	END TRY
	BEGIN CATCH
		ROLLBACK TRAN;
		DECLARE @ERROR_MESSAGE AS NVARCHAR(4000);
		SET @ERROR_MESSAGE = N'Failed to load gs].[CUSTOMER_LOG' + ISNULL(ERROR_MESSAGE(), '');
