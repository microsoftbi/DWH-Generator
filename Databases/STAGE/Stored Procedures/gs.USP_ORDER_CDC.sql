...
AS
	...
	SET @LOGSOURCE = N'[STAGE].[gs].[USP_ORDER_CDC]';
	
	EXEC META.dbo.USP_WRITELOG N'Start to load [STAGE].[gs].[ORDER_CDC]', @LOGSOURCE, N'N';
			...
			WHERE 1 = 1
				AND CASE
					...
					ELSE
						''
				END <> '';
			EXEC META.dbo.USP_WRITELOG N'Finish to load [STAGE].[gs].[ORDER_CDC', @LOGSOURCE, N'N';
		COMMIT TRAN;
	END TRY
	BEGIN CATCH
		ROLLBACK TRAN;
		DECLARE @ERROR_MESSAGE AS NVARCHAR(4000);
		SET @ERROR_MESSAGE = N'Failed to load [STAGE].[gs].[ORDER_CDC' + ISNULL(ERROR_MESSAGE(), '');
