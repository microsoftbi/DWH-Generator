...
AS
	...
	SET @LOGSOURCE = N'[STAGE].[gs].[USP_GAMELIST_LOG]';
	
	EXEC META.dbo.USP_WRITELOG N'Start to load [STAGE].[gs].[GAMELIST_LOG]', @LOGSOURCE, N'N';
	
	BEGIN TRY
		BEGIN TRAN;
	
			INSERT INTO [gs].[GAMELIST_LOG]
			(
				[LOAD_DTS],
				[LOAD_DTS_BATCH],
				[SEQUENCE_NO],
	...
	BEGIN CATCH
		...
		SET @ERROR_MESSAGE = N'Failed to load [STAGE].[gs].[GAMELIST_LOG]' + ISNULL(ERROR_MESSAGE(), '');
