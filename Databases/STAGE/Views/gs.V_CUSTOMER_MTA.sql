...
AS
	SELECT LOAD_DTS,
		...
		ROW_NUMBER() OVER (ORDER BY CONVERT(INT, CONVERT(NVARCHAR(30), [CustomerID]))) AS [SEQUENCE_NO],
		...
		CONVERT(INT, CONVERT(NVARCHAR(30), [CustomerID])) AS [CustomerID],
		CONVERT(NVARCHAR(30), [CustomerName]) AS [CustomerName],
		CONVERT(DATE, CONVERT(NVARCHAR(20), [Birth])) AS [Birth],
		CONVERT(INT, CONVERT(NVARCHAR(30), [Gender])) AS [Gender],
		CONVERT(NVARCHAR(50), [Address]) AS [Address]
	FROM [gs].[CUSTOMER];
GO
